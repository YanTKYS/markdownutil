 ...
 */
async function setupPresenter({ markdown = THREE_SLIDES, origin, openResult } = {}) {
  // Marp Coreの初期化は本物のブラウザのdocumentを前提とするため、
  // 代用DOMを差し込む前にレンダリングを済ませておく。
  if (markdown !== null) await render(markdown);
  const dom = installFakeDom();
  const win = installFakeWindow({ origin, openResult });
   ...
   * origin検証（PR#21）が有効な状態でも同期が続くことを確認するテストのため、
   * setupPresenter({ origin })で指定した本体オリジンと一致するevent.originを付ける。
   */
  const markReady = () => {
    win.emit('message', { source: els.currentFrame.contentWindow, origin, data: { type: 'ready' } });
    win.emit('message', { source: els.nextFrame.contentWindow, origin, data: { type: 'ready' } });
    const popup = win.lastOpened();
    if (popup) win.emit('message', { source: popup, origin, data: { type: 'ready' } });
  };
...
test('start: 投影用の別ウィンドウを開き、発表者ビューを発表中にする', async () => {
  ...
  try {
    ...
    assert.match(context.popup().written, /var host = window\.opener;/);
...
test('start: document.write()が失敗した場合はfalseを返し、空のウィンドウを残さない（v0.6.3のエラー処理）', async () => {
  ...
  try {
    ...
    try {
      ...
      assert.match(logs[0][0], /\[MarkdownUtil\] start: プレゼン用ウィンドウの初期化に失敗/);

      // 中途半端に開いたウィンドウを閉じている（空ウィンドウを残さない）。
      assert.equal(context.win.openedWindows[0].window.closed, true);
    } finally {
...
test('start〜同期: origin検証（PR#21）が有効でも、正しいオリジンからのreadyで発表者ビューが同期する', async () => {
  const context = await setupPresenter({ origin: 'http://127.0.0.1:8801' });
  try {
    start();
    context.markReady();
    ...
    assert.deepEqual(context.els.currentFrame.contentWindow.postedOrigins.filter(Boolean), [
      'http://127.0.0.1:8801',
      'http://127.0.0.1:8801',
      'http://127.0.0.1:8801',
    ]);
...
test('start〜同期: origin検証（PR#21）が有効な場合、異なるoriginからのメッセージは同期に反映されない', async () => {
  const context = await setupPresenter({ origin: 'http://127.0.0.1:8801' });
  try {
    start();
    context.markReady();
    ...
    assert.equal(context.els.positionEl.textContent, '1 / 3', '異なるoriginからのキー操作で位置が動いている');
  } finally {
    context.cleanup();
  }
});
