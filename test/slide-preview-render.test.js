...
test('buildFrameDocument: 既定はparentへpostMessageする', () => {
  ...
  assert.match(html, /var host = parent;/);
  assert.match(html, /if \(host\) host\.postMessage\(message, hostOrigin\);/);
  assert.ok(!html.includes('window.opener'));
...
test('buildFrameDocument: opener指定では別ウィンドウのopenerへ返す', () => {
  ...
  assert.match(html, /var host = window\.opener;/);
  assert.match(html, /if \(host\) host\.postMessage\(message, hostOrigin\);/);
});

test('buildFrameDocument: 送信元・オリジンを照合しないメッセージは受け付けない', () => {
  const html = buildFrameDocument();

  assert.match(html, /if \(!host \|\| event\.source !== host \|\| event\.origin !== self\.origin\) return;/);
});
