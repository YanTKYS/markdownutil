...
文書モードでは、Marp向けのfront matter（`marp:` / `theme:` / `paginate:` 等のYAML風ブロック）と
HTMLコメント（Marpのスピーカーノート、通常のMarkdownコメントの両方）は本文として表示しません。
どちらもMarp側が解釈するための情報であり、文書としての本文ではないためです（Word出力と同じ
前処理を共有しています）。エディタのMarkdown本文そのものは変更しません。
...
- 箇条書き・番号付きリスト（最大3階層のネストに対応。`3.`のように途中の番号から
  書き始めたリストは、プレビューと同じくその番号から数え始めます）
- 表（ヘッダー行・データ行・罫線。セル内の太字等の書式と、`---:`による桁揃えも維持）
...
│  ├─ markdown-engine.js  自作: markdown-itの初期化オプションとfront matter/コメント除去をpreview.js/word-export.jsで共有
...
- 編集支援ヘルプ（早見表）は、記法を思い出すための簡易な一覧です。Markdown仕様の網羅的な
  ...
  や追加項目の登録機能はありません。
- Marp向けのfront matterとHTMLコメントの除去（文書プレビュー・Word出力）は、コードブロック
  （```で囲んだ範囲）の中に書かれたHTMLコメントも対象になります。HTMLの記述例を
  コードブロックとして載せる場合はご注意ください。
