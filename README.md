...
Markdown編集領域の右上にある「? ヘルプ」を押すと、Markdown / Marpの代表的な記法を
タブ切替形式の早見表で確認できます。「Markdown」「Marp」それぞれについて、名称・簡単な説明・
記法例を掲載しており、記法例の右にある「コピー」でその記法だけをクリップボードへコピーできます
（コピー結果は既存のステータス表示で通知します）。Markdown仕様の詳細解説やチュートリアルでは
なく、書き方を忘れたときにその場で確認できる程度の軽い早見表です。`Esc`または閉じるボタンで
